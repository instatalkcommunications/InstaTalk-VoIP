...
NSString *DescriptionForObjectIR(NSObject *object, id locale, NSUInteger indent)
{
...
@implementation OrderedDictionaryIR
...
-(NSMutableDictionary*)getDictionay{
    return dictionary;
}
...
- (id)convertFromDictionary:(id)anObject {
	...
	for ( id key in anObject ) {
		...
		if ( [ [anObject objectForKey:key] isMemberOfClass: NSClassFromString( @"NSDictionary" ) ] ||
			...
			OrderedDictionaryIR *convertedObject = [[OrderedDictionaryIR alloc] convertFromDictionary:[anObject objectForKey:key] ];
...
- (NSString *)descriptionWithLocale:(id)locale indent:(NSUInteger)level
{
	...
	for (NSObject *key in self)
	{
		[description appendFormat:@"%@    %@ = %@;\n",
		 indentString,
		 DescriptionForObjectIR(key, locale, level),
		 DescriptionForObjectIR([self objectForKey:key], locale, level)];
	}
	[description appendFormat:@"%@}\n", indentString];
	return description;
}
