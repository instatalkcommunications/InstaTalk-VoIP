    ...
    if (![self isEditing]) {
        ...
        if(dest != nil) {
            ...
            if([ContactSelection getSelectionMode] != ContactSelectionModeMessage) {
                ...
                UICompositeViewDescription *view = [viewStack firstObject];
                
                
                if([view.content isEqualToString:@"CallBack"])
                {
                    //[[PhoneMainView instance] popToView:[CallBack compositeViewDescription]]; // Got to Chat and push ChatRoom
                    
                    CallBack *CallBackcontroller = DYNAMIC_CAST([[PhoneMainView instance] changeCurrentView:[CallBack compositeViewDescription]], CallBack);
                    
                    if(CallBackcontroller != nil) {
                        CallBackcontroller.txtDestination.text = [LinphoneAppDelegate FormatPhoneNumber:dest];
                    }
                }
                else if([view.content isEqualToString:@"ShowRates"] || [view.content isEqualToString:@"Accounts"])
                {
                    //[[PhoneMainView instance] popToView:[ShowRates compositeViewDescription]]; // Got to Chat and push ChatRoom
                    
                    ShowRates *CallBackcontroller = DYNAMIC_CAST([[PhoneMainView instance] changeCurrentView:[ShowRates compositeViewDescription]push:true ], ShowRates);
