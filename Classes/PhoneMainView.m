...
- (void)viewDidLoad {
    ...
    dispatch_queue_t queue = dispatch_queue_create("com.instatalkcommunications.voip", NULL);
    dispatch_async(queue, ^{
        
      [self CheckAndSaveDomain];  
    ...
    if (linphone_core_is_network_reachable(lc))
    {
        ...
        if (cfg){
            ...
            if (addr){
                ...
                linphone_proxy_config_normalize_number(cfg, [username cStringUsingEncoding:[NSString defaultCStringEncoding]], normalizedUserName, sizeof(normalizedUserName));
...
+ (CATransition*)getTransition:(UICompositeViewDescription *)old new:(UICompositeViewDescription *)new {
    ...
    if([old equal:[CallBack compositeViewDescription]]) {
